...
voice_dtmf_digit_t voice_dtmf_detector_process(
    ...
    for (size_t i = 0; i < num_samples; i++) {
        ...
        /* 更新所有 Goertzel 滤波器 */
        for (int j = 0; j < 4; j++) {
            goertzel_process(&detector->low_filters[j], sample);
            goertzel_process(&detector->high_filters[j], sample);
        }
        ...
        if (detector->sample_count >= detector->block_size) {
            ...
            for (int j = 0; j < 4; j++) {
                low_power[j] = goertzel_get_power(&detector->low_filters[j]);
                high_power[j] = goertzel_get_power(&detector->high_filters[j]);
                ...
                goertzel_reset(&detector->low_filters[j]);
                goertzel_reset(&detector->high_filters[j]);
            }
