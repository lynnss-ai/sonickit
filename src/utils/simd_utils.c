...
#if defined(_WIN32) || defined(_WIN64)
    /* Windows: use _aligned_malloc (including MinGW) */
...
static void int16_to_float_sse2(const int16_t *src, float *dst, size_t count) {
    ...
    /* Process aligned part (8 samples at a time) */
    for (; i + 8 <= count; i += 8) {
