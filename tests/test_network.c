...
static int test_rtp_packet_parse(void)
{
    ...
    /* Check header fields via header struct */
    TEST_ASSERT_EQ(parsed.header.payload_type, 111, "Payload type should match");
    TEST_ASSERT_EQ(parsed.payload_size, 160, "Payload size should match");
    TEST_ASSERT(parsed.header.marker, "Marker bit should be set");
...
static int test_rtp_sequence_numbers(void)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i > 0) {
            TEST_ASSERT_EQ(parsed.header.sequence, (uint16_t)(prev_seq + 1),
                          "Sequence number should increment");
        }
        prev_seq = parsed.header.sequence;
    }
...
static int test_jitter_buffer_put_get(void)
{
    ...
    /* Put some frames - jitter_buffer_put takes 6 params: jb, data, size, timestamp, sequence, marker */
    uint8_t frame1[100];
    memset(frame1, 1, sizeof(frame1));
    jitter_buffer_put(jb, frame1, 100, 0, 0, false);

    uint8_t frame2[100];
    memset(frame2, 2, sizeof(frame2));
    jitter_buffer_put(jb, frame2, 100, 960, 1, false);

    /* Get frames - jitter_buffer_get takes 5 params: jb, output, max_size, out_size, status */
    uint8_t output[100];
    size_t output_size;
    jitter_packet_status_t status;
    voice_error_t err = jitter_buffer_get(jb, output, sizeof(output), &output_size, &status);
