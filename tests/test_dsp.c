...
static int test_vad_detect_speech(void)
{
    ...
    bool is_speech = voice_vad_process(vad, loud_signal, 960);
    TEST_ASSERT(is_speech == true, "Should detect speech in loud signal");
    ...
    for (int i = 0; i < 10; i++) {
        is_speech = voice_vad_process(vad, silence, 960);
    }
    TEST_ASSERT(is_speech == false, "Should not detect speech in silence");
...
static int test_agc_amplify_quiet_signal(void)
{
    ...
    config.mode = VOICE_AGC_MODE_ADAPTIVE;
...
{
    voice_dtmf_generator_t *gen = voice_dtmf_generator_create(48000);
    TEST_ASSERT_NOT_NULL(gen, "DTMF generator creation should succeed");
    
    /* Generate tone for digit '5' */
    int16_t buffer[480];  /* 10ms */
    voice_error_t err = voice_dtmf_generator_generate(gen, '5', buffer, 480);
    TEST_ASSERT_EQ(err, VOICE_OK, "DTMF generation should succeed");
    
    /* Verify signal is not silent */
    float rms = calculate_rms(buffer, 480);
    TEST_ASSERT(rms > 5000.0f, "DTMF output should not be silent");
...
{
    voice_dtmf_detector_t *det = voice_dtmf_detector_create(48000);
    ...
    voice_dtmf_generator_generate(gen, '5', buffer, 960);
    
    /* Detect */
    char detected = voice_dtmf_detector_process(det, buffer, 960);
